...
def dist_between(start_logits, end_logits, device="cpu", max_seq_len=128):
    ...
    linear_func = torch.tensor(
        np.linspace(0, 1, max_seq_len, endpoint=False), requires_grad=False
    )
...
def lovasz_hinge(logits, labels, per_image=True, ignore=None, weights=None):
    """
