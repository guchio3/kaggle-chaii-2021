...
class TextBatchChooseTrainPipeline(Pipeline):
    def __init__(
        self,
        exp_id: str,
        config: Dict[str, Any],
    ...
    def _train(self) -> None:
        ...
        for fold, (trn_idx, val_idx) in enumerate(folds):
            ...
            if self.only_answer_text_training:
                raise Exception("not supported. self.only_answer_text_training")
            ...
            for epoch in range(self.num_epochs):
                model.textbatch_train_one_epoch(
                    device=self.device,
                    fold=fold,
                    epoch=epoch,
            ...
            fold_result_stat = f"best_val_auc for exp_id {self.exp_id} fold {fold} : {best_val_auc:.5f}"
